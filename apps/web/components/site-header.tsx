...
const ROLE_META: Record<Role, { key: 'roleGuest' | 'roleConsumer' | 'roleMerchant'; icon: typeof Eye }> = {
  ...
  return (
    <nav ref={ref} className="relative border-b border-border bg-card" aria-label={lang === 'ar' ? 'التصنيفات' : 'Categories'}>
      ...
      {open && (
        ...
        >
          <div className="mx-auto grid max-w-7xl grid-cols-[220px_1fr] gap-0">
            {/* Left: category list */}
              ...
              {CATEGORIES.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ChevronRight className={`size-3.5 opacity-40 ${lang === 'ar' ? 'rotate-180' : ''}`} />
                  </button>
                )
              })}
            </div>
            ...
            <div className="p-5">
              <div className="mb-3 flex items-center gap-2">
                ...
                >
                  {t('seeAll')}
                  <ChevronRight className={`size-3.5 ${lang === 'ar' ? 'rotate-180' : ''}`} />
                </Link>
              </div>
                  ...
                  {featuredProducts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="line-clamp-2 text-xs font-medium text-foreground leading-snug">{name}</p>
      ...
      <div className="bg-primary text-primary-foreground">
        <div className="mx-auto flex h-9 max-w-7xl items-center justify-between px-4 text-xs">
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Role switcher */}
            <Dropdown
              icon={<RoleIcon className="size-3.5" />}
              label={
                <span className="flex items-center gap-1">
                  <span className="hidden opacity-80 sm:inline">{t('viewingAs')}:</span>
                  {t(ROLE_META[role].key)}
                </span>
              }
            >
              {(close) =>
                (['guest', 'consumer', 'merchant'] as Role[]).map((r) => {
              ...
              {(close) =>
                (['ar', 'en'] as Lang[]).map((l) => (
                  <button
                    ...
                    className={`flex w-full items-center px-3 py-2 text-start text-sm hover:bg-accent ${
