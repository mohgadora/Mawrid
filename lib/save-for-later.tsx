...
type SavedItem = { productId: string; qty: number; snapshot?: CartProductSnapshot }

type SavedContextValue = {
  saved: SavedItem[]
  saveItem: (productId: string, qty: number, snapshot?: CartProductSnapshot) => void
...
export function SaveForLaterProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveItem = useCallback((productId: string, qty: number, snapshot?: CartProductSnapshot) => {
    setSaved((prev) => {
