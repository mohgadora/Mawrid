...
import { ORDER_FLOW, type OrderEvent, type OrderStatus } from '@/lib/order-types'
...
export function StatusChip({ status, size = 'md' }: { status: string; size?: 'sm' | 'md' }) {
  const { t } = useI18n()
  const meta = STATUS_META[status as OrderStatus] as Meta | undefined
  const Icon = meta?.icon ?? CircleDashed
  const chipClass = meta?.chip ?? 'bg-muted text-muted-foreground'
  const label = meta ? t(meta.dictKey) : status
  return (
    <span
      className={cn(
